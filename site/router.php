 ...
 * @return  array
 */
function DzvideoBuildRoute(&$query)
{
    $segments = array();
    ...
    //var_dump($query, $mView, $mId);
    if (isset($query['view']))
    {
        $view = $query['view'];

        if (empty($query['Itemid']) || empty($menuItem) || $menuItem->component != 'com_dzvideo')
        {
            $segments[] = $query['view'];
        }
    ...
    // are we dealing with an video that is attached to a menu item?
    ...
    if (isset($view) and ($view == 'category' or $view == 'video'))
    {
        if ($mId != (int) $query['id'] || $mView != $view)
        {
            if ($view == 'video' && isset($query['catid']))
            {
                $catid = $query['catid'];
            }
 ...
 */
function DzvideoParseRoute($segments)
{
    $vars = array();
